...
class MainWindow (QMainWindow):
    ...
    def __init__ (self, parent = None):
        ...
        self.pilatus1m=TwoDDetector('Pilatus1M')
        np.seterr(invalid='ignore',divide='ignore')
        self.mcaDirName='vortex'
        self.pil1mDirName='pilatus1m'
        ...
        self.ui.refAbsFacTableWidget.setHorizontalHeaderLabels(QStringList()<<'Absorber'<<'Factor')
        ...
        self.connect(self.ui.actionCurvature, SIGNAL('triggered()'), self.calcur)
       ...
       # self.connect(self.ui.scanListWidget, SIGNAL('itemClicked(QListWidgetItem*)'),self.scanListChanged)
        ...
        self.connect(self.ui.refAbsAddPushButton, SIGNAL('clicked()'), self.insAbsFacRow)
        self.connect(self.ui.refAbsRemPushButton, SIGNAL('clicked()'), self.remAbsFacRow)
       ...
       # self.connect(self.ui.pilGIDPatchPushButton, SIGNAL('clicked()'),self.pilGIDpatchData)
        ...
        self.connect(self.ui.mergePushButton,  SIGNAL('clicked()'), self.plotMergeDia)
    ...
    def readSpecFile(self):
        ...
        if self.specData['NumOfScans']==0:
            ...
            if self.specData['Error']:
                self.messageBox('Warning:: '+self.specData['Message'])
            ...
            if np.any([self.specPar[i]['Detector']=='Pilatus1M' for i in range(1,self.specData['NumOfScans']+1)]):
                if QDir(self.directory+'/'+self.pil1mDirName).exists()==True:
                    self.pil1mDir=self.directory+'/'+self.pil1mDirName
                else:
                    self.messageBox('Warning:: The default Piltus1M directory not found, please choose!!')
                    self.pil1mDir=QFileDialog.getExistingDirectory(caption='Open Pilatus1M Directory')
                self.pil1mfhead=str(self.pil1mDir)+'/'+self.specFileName.split('/')[-1]+'_SCAN-'
                self.pil1mftail='.tif'
                print "Pilatus1M Images Found!!"
    ...
    def scanListChanged(self):
        ...
        if self.checkSameScans()==False or self.selectedScanNums==[]:
         ...
         #       self.ui.spNComboBox.setCurrentIndex(len(self.specData[self.selectedScanNums[0]]['ScanVar'])-2)   
            ...
            if self.specPar[self.selectedScanNums[0]]['Detector']=='Pilatus1M':
                self.updateCcdImageList()
                self.ui.PlotWidget.setCurrentIndex(0)
                self.ui.imagesLabel.setText('Pilatus1M:')
        self.ui.statusBar.showMessage('Done')
        ...
        if self.checkSameScans()==False:
            ...
            for i in self.selectedScanNums:
                ...
                try:
                    ...
                    self.scanCenter[i]=float(format(self.specFitPar[0],'.3f'))
                except:
                    self.peakPos='%.4f'%x[np.argmax(y)]
    ...
    def checkSameQzs(self,qzvalues):
        res=self.ui.qzResSpinBox.value()
        print res
        return all(np.abs(x-qzvalues[0])<pow(10,-res) for x in qzvalues)
    ...
    def saveMcaData(self):
        ...
        for i in self.selectedMcaScanNums:
            self.fmcaName=self.saveMcaFileName+str(self.ui.imageListWidget.item(i).text().split('\t')[0])+'_'+'F'+str(i)+'_'+'%.4f'%self.mcaPar[i]['Q'][2]+'_mca.txt'
    ...
    def unSelectedAllImages(self):
        if self.ui.imageSelectAllCheckBox.checkState()!=0 and self.specPar[self.selectedScanNums[0]]['Detector']=='Pilatus1M':
            self.disconnect(self.ui.imageListWidget,SIGNAL('itemSelectionChanged()'),self.imageSelectedScanChanged)
    ...
    def imageSelectedScanChanged(self):
        ...
        elif self.specPar[self.selectedScanNums[0]]['Detector']=='Pilatus1M':
            self.det='Pilatus1M'
            ...
            try:
                ...
                for j in range(self.numFrames[i]):
                    ...
                    try:
                        ...
                        self.ccdX_off.append(self.specPar[i]['PD1M_X'])
                        self.ccdY_off.append(self.specPar[i]['PD1M_Z'])
...
#                self.messageBox('Warning:: #S '+str(i+1)+' has been canceled or no data found.')
    ...
    def ccdSelectedScanChanged(self):
        ...
        j=0
        self.progressDialog=QProgressDialog('Reading Pilatus1M Images','Abort',0,100)
        ...
        for i in self.selectedCcdFramesNums:
            if len(self.ui.backgroundListWidget.selectedItems())==0:
                self.pilatus1m.openFile(self.ccdFileNames[i],bad_pix=bad_pix)
                self.ccdData[i]=self.pilatus1m.imageData
                self.ccdLogData[i]=np.log10(self.ccdData[i]+1)
                self.ccdErrorData[i]=self.pilatus1m.errorData
                self.xyzformat='x=%.3f,y=%.3f,z=%d'
            else:
                self.pilatus1m.openFile(self.ccdFileNames[i],bad_pix=bad_pix)
                self.ccdData[i]=self.pilatus1m.imageData
                self.ccdErrorData[i]=self.pilatus1m.errorData
                self.pilatus1m.sumFiles({i:self.ccdData[i]},{i:self.ccdErrorData[i]},absfac=self.absfac,absnum=[self.ccdSelected_AbsNum[j]],mon=[self.ccdSelectedMonc[j]]) # For the Normalization Calculation Only!!
                self.ccdData[i]=self.pilatus1m.imageData-fac*self.bgData
                ...
                self.ccdErrorData[i]=np.sqrt(self.pilatus1m.errorData**2+fac**2*self.bgError**2)
                j=j+1
                self.xyzformat='x=%.3f,y=%.3f,z=%.2e'
            self.progressDialog.setLabelText('Reading Pilatus1M Frame #'+str(i))     
            self.updateProgress()
        ...
        for i in self.selectedCcdFramesNums:
            res='.'+str(self.ui.qzResSpinBox.value())+'f'
            print res
            if float(format(self.ccdFileQzs[i], res)) in self.ccdFrameNumsWithSameQz:                
                self.ccdFrameNumsWithSameQz[float(format(self.ccdFileQzs[i], res))].append(i)
                self.ccdFrameAbsWithSameQz[float(format(self.ccdFileQzs[i], res))].append(self.ccd_AbsNum[i])
            else:
                self.ccdFrameNumsWithSameQz[float(format(self.ccdFileQzs[i], res))]=[i]
                self.ccdFrameAbsWithSameQz[float(format(self.ccdFileQzs[i], res))]=[self.ccd_AbsNum[i]]
        try:
            ...
            self.imageMax=100
            self.imageMin=0
        if self.ui.gixFixConCheckBox.checkState()==0:
...
#            self.messageBox('Warning:: Some scans maybe missing')
    ...
    def bgSelectionChanged(self):
        ...
        if self.det=='Pilatus1M':
            ...
            self.pilatus1m.sumFiles(self.selectedBgData,self.selectedBgError,absfac=self.absfac,absnum=self.selectedBgAbsNum,mon=self.selectedBgMonc)
            self.bgData=self.pilatus1m.imageData
            self.bgError=self.pilatus1m.errorData
        elif self.det=='Pilatus':
            for i in self.selectedBgFrameNums:
...
#            self.ui.gixSumCheckBox.setCheckState(2)
    ...
    def updateCcdPlotData(self):
        if len(self.selectedCcdFramesNums)!=0:
            ...
            ccdXMax=float(self.pilatus1m.NCOLS)*np.ones_like(self.selectedCcdFramesNums)
            ccdYMax=float(self.pilatus1m.NROWS)*np.ones_like(self.selectedCcdFramesNums)
            self.pix_size=0.172
            self.origin='upper'
            self.ccdcoord=self.format_coord
            xlabel='PixX'
            ylabel='PixY'
            ...
            if self.ui.gixSpecCheckBox.checkState()!=0:
                ...
                self.xoff=np.array(map(int, np.array(self.ccdSelectedXoff)/self.pix_size))
                self.yoff=np.array(map(int, np.array(self.ccdSelectedYoff)/self.pix_size))
                ...
                self.xoff=np.array(map(int,float(self.ui.gixCcd_OffLineEdit.text().split(',')[0])*np.ones_like(self.selectedCcdFramesNums)/self.pix_size))
                self.yoff=np.array(map(int,float(self.ui.gixCcd_OffLineEdit.text().split(',')[-1])*np.ones_like(self.selectedCcdFramesNums)/self.pix_size))
            ...
            if str(self.ui.gixAxesComboBox.currentText())=='Angles':
                ...
                ccdXMin=(ccdXMin-(self.xcenter-self.xoff))*self.pix_size*180.0/np.pi/self.distance
                ccdXMax=(ccdXMax-(self.xcenter-self.xoff))*self.pix_size*180.0/np.pi/self.distance
                ccdYMin=(((self.ycenter-self.yoff)-ccdYMin)*self.pix_size-self.ccdSelected_Sh)*180.0/np.pi/self.distance
                ccdYMax=(((self.ycenter-self.yoff)-ccdYMax)*self.pix_size-self.ccdSelected_Sh)*180.0/np.pi/self.distance
            elif str(self.ui.gixAxesComboBox.currentText())=='Q':
                xlabel='Qxy [1/Angs]'
                ylabel='Qz [1/Angs]'
                ccdXMin=4.0*np.pi*np.sin((ccdXMin-(self.xcenter-self.xoff))*self.pix_size/self.distance/2.0)/self.wavelength
                ccdXMax=4.0*np.pi*np.sin((ccdXMax-(self.xcenter-self.xoff))*self.pix_size/self.distance/2.0)/self.wavelength
                ccdYMin=2.0*np.pi*(np.sin((((self.ycenter-self.yoff)-ccdYMin)*self.pix_size-self.ccdSelected_Sh)/self.distance)+np.sin(self.alpha))/self.wavelength
                ccdYMax=2.0*np.pi*(np.sin((((self.ycenter-self.yoff)-ccdYMax)*self.pix_size-self.ccdSelected_Sh)/self.distance)+np.sin(self.alpha))/self.wavelength
            ...
            if self.ui.gixSumCheckBox.checkState()!=0:
                if self.checkSameQzs(self.ccdSelectedQzs)==True:
                    ...
                    if self.ui.gixLogIntCheckBox.checkState()!=0:
                        p=ax.imshow(self.logData,interpolation='nearest',extent=self.extent,vmax=np.log10(vmax),vmin=np.log10(vmin),cmap=cmap,aspect='equal', origin=self.origin) 
                        self.Zdata=self.logData
                    else:
                        p=ax.imshow(self.sumData,interpolation='nearest',extent=self.extent,vmax=vmax,vmin=vmin,cmap=cmap,aspect='equal', origin=self.origin)
                        self.Zdata=self.sumData 
                    ax.format_coord=self.ccdcoord
                ...
                for i in self.selectedCcdFramesNums[:N]:
                    ...
                    if self.ui.gixLogIntCheckBox.checkState()!=0:
                        p=ax[i].imshow(self.ccdLogData[i],interpolation='nearest',extent=self.extent,vmax=np.log10(vmax),vmin=np.log10(vmin),cmap=cmap,aspect='equal',origin=self.origin)
                        self.Zdata=self.ccdLogData[i] 
                    else:
                        p=ax[i].imshow(self.ccdData[i],interpolation='nearest',extent=self.extent,vmax=vmax,vmin=vmin,cmap=cmap,aspect='equal',origin=self.origin)
                        self.Zdata=self.ccdData[i]
                    ax[i].format_coord=self.ccdcoord
                    self.ui.gixMplWidget.canvas.fig.colorbar(p)
                    num=num+1
            self.ui.gixMplWidget.canvas.draw()
    ...
    def pilGIDpatchData(self):
       ...
       # print self.pilhintLQDatadic[LQkeys[0]][0][0][0]
        if self.pilhintLQDatadic[LQkeys[0]][0][0][0]>2:  # Area and polarization corrections; this threshhold needs to be checked. 
            for i in range(len(LQkeys)):
                for j in range(len(self.pilhintLQDatadic[LQkeys[i]])):
                    self.pilhintLQDatadic[LQkeys[i]][j][:,2]=self.pilhintLQDatadic[LQkeys[i]][j][:,2]*np.sin(self.pilhintLQDatadic[LQkeys[i]][j][:,0]*np.pi/180)/np.sin(self.pilhintLQDatadic[LQkeys[i]][0][0][0]*np.pi/180)/(1-(np.sin(self.pilhintLQDatadic[LQkeys[i]][j][:,0]*np.pi/180)*np.cos(self.pilhintLQDatadic[LQkeys[i]][j][:,1]*np.pi/180))**2)
                    self.pilhintLQDatadic[LQkeys[i]][j][:,3]=self.pilhintLQDatadic[LQkeys[i]][j][:,3]*np.sin(self.pilhintLQDatadic[LQkeys[i]][j][:,0]*np.pi/180)/np.sin(self.pilhintLQDatadic[LQkeys[i]][0][0][0]*np.pi/180)/(1-(np.sin(self.pilhintLQDatadic[LQkeys[i]][j][:,0]*np.pi/180)*np.cos(self.pilhintLQDatadic[LQkeys[i]][j][:,1]*np.pi/180))**2)
            for i in range(len(MQkeys)):
                for j in range(len(self.pilhintMQDatadic[MQkeys[i]])):
                    self.pilhintMQDatadic[MQkeys[i]][j][:,2]=self.pilhintMQDatadic[MQkeys[i]][j][:,2]*np.sin(self.pilhintMQDatadic[MQkeys[i]][j][:,0]*np.pi/180)/np.sin(self.pilhintMQDatadic[MQkeys[i]][0][0][0]*np.pi/180)/(1-(np.sin(self.pilhintMQDatadic[MQkeys[i]][j][:,0]*np.pi/180)*np.cos(self.pilhintMQDatadic[MQkeys[i]][j][:,1]*np.pi/180))**2)
                    self.pilhintMQDatadic[MQkeys[i]][j][:,3]=self.pilhintMQDatadic[MQkeys[i]][j][:,3]*np.sin(self.pilhintMQDatadic[MQkeys[i]][j][:,0]*np.pi/180)/np.sin(self.pilhintMQDatadic[MQkeys[i]][0][0][0]*np.pi/180)/(1-(np.sin(self.pilhintMQDatadic[MQkeys[i]][j][:,0]*np.pi/180)*np.cos(self.pilhintMQDatadic[MQkeys[i]][j][:,1]*np.pi/180))**2)
            for i in range(len(HQkeys)):
                for j in range(len(self.pilhintHQDatadic[HQkeys[i]])):
                    self.pilhintHQDatadic[HQkeys[i]][j][:,2]=self.pilhintHQDatadic[HQkeys[i]][j][:,2]*np.sin(self.pilhintHQDatadic[HQkeys[i]][j][:,0]*np.pi/180)/np.sin(self.pilhintHQDatadic[HQkeys[i]][0][0][0]*np.pi/180)/(1-(np.sin(self.pilhintHQDatadic[HQkeys[i]][j][:,0]*np.pi/180)*np.cos(self.pilhintHQDatadic[HQkeys[i]][j][:,1]*np.pi/180))**2)
                    self.pilhintHQDatadic[HQkeys[i]][j][:,3]=self.pilhintHQDatadic[HQkeys[i]][j][:,3]*np.sin(self.pilhintHQDatadic[HQkeys[i]][j][:,0]*np.pi/180)/np.sin(self.pilhintHQDatadic[HQkeys[i]][0][0][0]*np.pi/180)/(1-(np.sin(self.pilhintHQDatadic[HQkeys[i]][j][:,0]*np.pi/180)*np.cos(self.pilhintHQDatadic[HQkeys[i]][j][:,1]*np.pi/180))**2)
       # print self.pilhintLQDatadic
...
#                self.pil_ylabel= 'Theta [Degrees]'
    ...
    def format_coord(self,x,y):  # cursor interactive 
        ...
        row = int(np.abs((y-self.extent[3]))*numrows/np.abs((self.extent[3]-self.extent[2])))
...
#        cmap=str(self.ui.gixCMapComboBox.currentText())
        ...
        if self.ui.gixMergeSameQzCheckBox.checkState()!=0:  #merge same qz
            j=0
            for key in sorted(self.ccdFrameNumsWithSameQz.keys()):   #key is the qz
                self.scanframe[key]={}
                for item in self.ccdFrameNumsWithSameQz[key]:
                    ekey=int(str(self.ui.imageListWidget.item(item).text()).split('\t')[0].split('#')[1])  #ekey is the scan number
                    if ekey in self.scanframe[key]:
                        self.scanframe[key][ekey].append(int(str(self.ui.imageListWidget.item(item).text()).split('\t')[1].split('#')[1]))  #add frame number
                    else:
                ...
                if all(x==self.ccdFrameAbsWithSameQz[key][0] for x in self.ccdFrameAbsWithSameQz[key]):     #for frames with the same absorber
                    ...
                    j=j+1
                else:   #for frames with different absorbers
    ...
    def refQzListSelectionChanged(self):
        ...
        if self.det=='Pilatus1M':
           ...
           # print self.ccdX, self.ccd_Gl2, self.ccd_Tth, self.wavelength, self.selFrameNums
          #  print self.ccd_Tth, self.ccdAlpha, self.ccd_Sh, self.selFrameNums
            ...
            cenx=self.ccdX[self.selFrameNums[0]]-int((dist+self.ccd_Gl2[self.selFrameNums[0]])*(np.tan(self.ccd_Tth[self.selFrameNums[0]]*np.pi/180)-np.tan(2*np.arcsin(self.ccd_Wavelength[self.selFrameNums[0]]/np.pi*1.9236/4)))/self.pix_size)
            ceny=self.ccdY[self.selFrameNums[0]]-int((self.ccdAlpha[self.selFrameNums[0]]*dist+self.ccd_Sh[self.selFrameNums[0]])/self.pix_size)
            ...
            self.selErrorData={}
            if self.dir=='H':
                self.bg=self.bg*self.ccd_Dist[self.selFrameNums[0]]*np.pi/180/self.pix_size/slitx
            else:
                self.bg=self.bg*self.ccd_Dist[self.selFrameNums[0]]*np.pi/180/self.pix_size/slity   
            for i in self.selFrameNums:
                self.selData[i]=self.ccdData[i]
                self.selErrorData[i]=self.ccdErrorData[i]
            self.pilatus1m.setROI(self.selData,self.selErrorData,absfac=self.absfac,absnum=self.absnum,slit=self.slit,cen=self.cen,bg=self.bg,dir=self.dir,mon=self.mon)
            slit=[self.slit[1], self.slit[0]]
            cen=[self.cen[1], self.cen[0]]
            self.ax={}
            if self.dir=='H':
                self.refextent=[self.pilatus1m.bglcen-slit[1], self.pilatus1m.bgrcen+slit[1]+1, cen[0]+slit[0]+1, cen[0]-slit[0]]
                self.ax[1]=self.ui.refADDataPlotWidget.canvas.fig.add_subplot(2,1,1)
                self.vmax=np.max(self.pilatus1m.imageData[cen[0]-slit[0]:cen[0]+slit[0]+1, self.pilatus1m.bglcen-slit[1]:self.pilatus1m.bgrcen+slit[1]+1])
                self.vmin=np.max(self.pilatus1m.imageData[cen[0]-slit[0]:cen[0]+slit[0]+1, self.pilatus1m.bglcen-slit[1]:self.pilatus1m.bgrcen+slit[1]+1])
                self.refZdata=self.pilatus1m.imageData
                p=self.ax[1].imshow(self.pilatus1m.imageROI[cen[0]-slit[0]:cen[0]+slit[0]+1, self.pilatus1m.bglcen-slit[1]:self.pilatus1m.bgrcen+slit[1]+1], origin='upper', aspect='auto', vmax=self.vmax, cmap=cmap, extent=self.refextent, interpolation='nearest')
                self.ax[1].set_ylabel('w/o Corr')
                self.ax[1].format_coord=self.format_coord
            else:
                self.refextent=[cen[1]-slit[1], cen[1]+slit[1]+1, self.pilatus1m.bgdcen-slit[0], self.pilatus1m.bgucen+slit[0]+1]
                self.ax[1]=self.ui.refADDataPlotWidget.canvas.fig.add_subplot(1,2,1)            
                self.vmax=np.max(self.pilatus1m.imageData[self.pilatus1m.bgdcen-slit[0]:self.pilatus1m.bgucen+slit[0]+1,cen[1]-slit[1]:cen[1]+slit[1]+1])
                self.vmin=np.max(self.pilatus1m.imageData[self.pilatus1m.bgdcen-slit[0]:self.pilatus1m.bgucen+slit[0]+1,cen[1]-slit[1]:cen[1]+slit[1]+1])
                self.refZdata=self.pilatus1m.imageData
                p=self.ax[1].imshow(self.pilatus1m.imageROI[self.pilatus1m.bgdcen-slit[0]:self.pilatus1m.bgucen+slit[0]+1,cen[1]-slit[1]:cen[1]+slit[1]+1], origin='upper', aspect='auto', vmax=self.vmax, cmap=cmap, extent=self.refextent, interpolation='nearest')
                self.ax[1].set_ylabel('w/o Corr')
                self.ax[1].format_coord=self.format_coord
    ...
    def refAnalyze(self):
        ...
        if self.det=='Pilatus1M':
            ...
            if self.ui.refCenCheckBox.checkState()!=0:
                self.cen=[int(self.ui.refCenLineEdit.text().split(',')[0]),int(self.ui.refCenLineEdit.text().split(',')[1])]
                self.cenfit=1
            self.par=self.pilatus1m.peakFind(self.selData,self.selErrorData,slit=self.slit,cen=self.cen,absfac=self.absfac,absnum=self.absnum,fac=fac,mon=self.mon,cenfit=self.cenfit)
            self.cen=[int(np.floor(self.par[1])), int(np.floor(self.par[3]))]
            slit=[self.slit[1], self.slit[0]]
            cen=[self.cen[1], self.cen[0]]
            bfac=float(self.ui.refBPCFacLineEdit.text())
            try:
                sig,sigerr,lbg,lbgerr,rbg,rbgerr=self.pilatus1m.badPix_corr(par=self.par,slit=self.slit,cen=self.cen,bad=1,bfac=bfac,dir=self.dir,bg=self.bg,plot=0)
                #print sig, lbg, rbg
                #sig,sigerr,lbg,lbgerr,rbg,rbgerr=self.pilatus1m.sumROI(slit=self.slit,cen=self.cen,dir=self.dir,bg=self.bg)
            except:
                self.messageBox('Warning:: Please change to different search window by +/- 0.5.')
            if self.dir=='H':
                self.ui.refBadPixListWidget.addItem('------Signal------')
                for items in self.pilatus1m.badSig:
                    self.ui.refBadPixListWidget.addItem(str(items[0])+'\t'+str(items[1]))
                self.ui.refBadPixListWidget.addItem('------Left BG------')
                for items in self.pilatus1m.badLeft:
                    self.ui.refBadPixListWidget.addItem(str(items[0])+'\t'+str(items[1]))
                self.ui.refBadPixListWidget.addItem('------Right BG------')
                for items in self.pilatus1m.badRight:
                    self.ui.refBadPixListWidget.addItem(str(items[0])+'\t'+str(items[1]))
            else:
                self.ui.refBadPixListWidget.addItem('------Signal------')
                for items in self.pilatus1m.badSig:
                ...
                for items in self.pilatus1m.badUp:
                    self.ui.refBadPixListWidget.addItem(str(items[0])+'\t'+str(items[1]))
                self.ui.refBadPixListWidget.addItem('-----Down BG------')
                for items in self.pilatus1m.badDown:
                    self.ui.refBadPixListWidget.addItem(str(items[0])+'\t'+str(items[1]))
            self.pilatus1m.setROI(self.pilatus1m.imageData,self.pilatus1m.errorData,slit=self.slit,cen=self.cen,absfac=self.absfac,absnum=self.absnum,bg=self.bg,dir=self.dir,mon=None)  
            if self.dir=='H':            
                self.ax[2]=self.ui.refADDataPlotWidget.canvas.fig.add_subplot(2,1,2)
                self.refextent=[self.pilatus1m.bglcen-slit[1], self.pilatus1m.bgrcen+slit[1]+1, cen[0]+slit[0]+1, cen[0]-slit[0]]
                self.vmax=np.max(self.pilatus1m.imageData[cen[0]-slit[0]:cen[0]+slit[0]+1, self.pilatus1m.bglcen-slit[1]:self.pilatus1m.bgrcen+slit[1]+1])
                self.vmin=np.min(self.pilatus1m.imageData[cen[0]-slit[0]:cen[0]+slit[0]+1, self.pilatus1m.bglcen-slit[1]:self.pilatus1m.bgrcen+slit[1]+1])
                p=self.ax[2].imshow(self.pilatus1m.imageROI[cen[0]-slit[0]:cen[0]+slit[0]+1, self.pilatus1m.bglcen-slit[1]:self.pilatus1m.bgrcen+slit[1]+1], origin='upper', aspect='auto', vmax=self.vmax, cmap=cmap, extent=self.refextent, interpolation='nearest')
                self.ax[2].set_ylabel('w Corr')
                self.ax[2].format_coord=self.format_coord
            else:
                self.ax[2]=self.ui.refADDataPlotWidget.canvas.fig.add_subplot(1,2,2)
                self.ax[2].clear()
                self.refextent=[cen[1]-slit[1], cen[1]+slit[1]+1, self.pilatus1m.bgdcen-slit[0], self.pilatus1m.bgucen+slit[0]+1]
                self.vmax=np.max(self.pilatus1m.imageData[self.pilatus1m.bgdcen-slit[0]:self.pilatus1m.bgucen+slit[0]+1,cen[1]-slit[1]:cen[1]+slit[1]+1])
                self.vmin=np.min(self.pilatus1m.imageData[self.pilatus1m.bgdcen-slit[0]:self.pilatus1m.bgucen+slit[0]+1,cen[1]-slit[1]:cen[1]+slit[1]+1])
                p=self.ax[2].imshow(self.pilatus1m.imageROI[self.pilatus1m.bgdcen-slit[0]:self.pilatus1m.bgucen+slit[0]+1,cen[1]-slit[1]:cen[1]+slit[1]+1], origin='upper', aspect='auto', vmax=self.vmax, cmap=cmap, extent=self.refextent, interpolation='nearest')
                self.ax[2].set_ylabel('w Corr')
                self.ax[2].format_coord=self.format_coord
        elif self.det=='Pilatus':
            self.ui.refBadPixListWidget.clear()
    ...
    def insAbsFacRow(self): #add a row in AbsFac table
        row=self.ui.refAbsFacTableWidget.rowCount() 
        self.ui.refAbsFacTableWidget.insertRow(row)
        for i in range(2):
            self.ui.refAbsFacTableWidget.setItem(row,i,QTableWidgetItem('0/1'.split('/')[i]))
                
    def remAbsFacRow(self): #remove one or multiple rows in AbsFac table
        rmrows=self.ui.refAbsFacTableWidget.selectionModel().selectedRows()   
        removerows=[]
        for rmrow in rmrows:
            removerows.append(self.ui.refAbsFacTableWidget.row(self.ui.refAbsFacTableWidget.itemFromIndex(rmrow)))
            removerows.sort(reverse=True)
        if len(removerows)==0:
            self.messageBox('Warning:: No row is selected')
        else:
            for i in range(len(removerows)):
               self.ui.refAbsFacTableWidget.removeRow(removerows[i])

            
    def updateAbsFac(self): #update the abs factors
        row=self.ui.refAbsFacTableWidget.rowCount() 
        self.absfac=[]
        for i in range(row):
            self.absfac.append([int(str(self.ui.refAbsFacTableWidget.item(i,0).text())),float(str(self.ui.refAbsFacTableWidget.item(i,1).text()))])
        self.absfac=np.array(self.absfac)
       # print self.absfac
   
    def updateRefDataList(self):
        if self.refpatchindex==0: #regular ref
            comments=str(self.refQzSelectedItems[0].text()).split('\t')[1:]
            if self.ui.refAbsFacCheckBox.checkState()!=0:
                self.updateAbsFac()
                absnum=int(comments[1].split('[')[-1].split(']')[0])
              #  print absnum, np.where(self.absfac[:,0]==absnum)[0]
                if len(np.where(self.absfac[:,0]==absnum)[0])!=1:
                    self.messageBox('Warning: either this absorber is not listed in the absorber factor table or there are duplicated absorbers in the table!!!')
                else:
                    absfac=self.absfac[np.where(self.absfac[:,0]==absnum)[0][0]][1]
                    print absfac, self.fsig
                    self.refData.append([float(str(self.refQzSelectedItems[0].text()).split()[1]),self.fsig*absfac,self.fsigerr*absfac])
                    self.refInfo.append('#'+comments[0]+'\t'+comments[1]+'\t#Abs factor:'+str(absfac)+'\t#Comments:'+str(self.ui.refComLineEdit.text()))
                    self.ui.refDataListWidget.addItem('%.4f'%float(str(self.refQzSelectedItems[0].text()).split()[1])+'\t'+'%.4e'%(self.fsig*absfac)+'\t'+'%.4e'%(self.fsigerr*absfac)+'\t'+self.refInfo[-1])
            else:
    ...
    def updateCcdCutData(self):        
        ...
        if self.ui.gixSumCheckBox.checkState()!=0:
            ...
            if self.ui.gixCutDirComboBox.currentText()=='H Cut':
                self.pilatus1m.plotVint(self.selCutCcdData[-1],self.selCutCcdErrorData[-1],absfac=self.absfac,absnum=self.ccdSelected_AbsNum[0],cen=[self.xcenter[0]-self.xoff[0],self.ycenter[0]-self.yoff[0]],  hroi=None,  vroi=[max(int(ini)-1,0), min(int(fin)-1,self.pilatus1m.NROWS)], ax_type=str(self.ui.gixAxesComboBox.currentText()), wavelength=self.wavelength[0],s2d_dist=self.distance[0],sh=self.ccdSelected_Sh[0],alpha=self.alpha[0], mon=None)
                self.cutData[-1]=self.pilatus1m.vintData
            elif self.ui.gixCutDirComboBox.currentText()=='Qz Cut':
                ini=-int((self.distance[0]*np.arcsin(ini*self.wavelength[0]/2.0/np.pi-np.sin(self.alpha[0]))+self.ccdSelected_Sh[0])/self.pix_size)+self.ycenter[0]-self.yoff[0]
                fin=-int((self.distance[0]*np.arcsin(fin*self.wavelength[0]/2.0/np.pi-np.sin(self.alpha[0]))+self.ccdSelected_Sh[0])/self.pix_size)+self.ycenter[0]-self.yoff[0]                
                print ini, fin
                self.pilatus1m.plotVint(self.selCutCcdData[-1],self.selCutCcdErrorData[-1],absfac=self.absfac,absnum=self.ccdSelected_AbsNum[0], cen=[self.xcenter[0]-self.xoff[0],self.ycenter[0]-self.yoff[0]],  hroi=None,  vroi=[max(fin-1,0), min(ini-1,self.pilatus1m.NROWS)], ax_type='Q', wavelength=self.wavelength[0],s2d_dist=self.distance[0],sh=self.ccdSelected_Sh[0],alpha=self.alpha[0], mon=None)
                self.cutData[-1]=self.pilatus1m.vintData
            elif self.ui.gixCutDirComboBox.currentText()=='V Cut':
                self.pilatus1m.plotHint(self.selCutCcdData[-1],self.selCutCcdErrorData[-1],absfac=self.absfac,absnum=self.ccdSelected_AbsNum[0],cen=[self.xcenter[0]-self.xoff[0],self.ycenter[0]],  hroi=[max(int(ini)-1,0), min(int(fin)-1,self.pilatus1m.NCOLS)],  vroi=None, ax_type=str(self.ui.gixAxesComboBox.currentText()), wavelength=self.wavelength[0],s2d_dist=self.distance[0],sh=self.ccdSelected_Sh[0],alpha=self.alpha[0], mon=None)
                self.cutData[-1]=self.pilatus1m.hintData
            else:
                ini=int(2.0*self.distance[0]*np.arcsin(ini*self.wavelength[0]/4.0/np.pi)/self.pix_size)+self.xcenter[0]-self.xoff[0]
                fin=int(2.0*self.distance[0]*np.arcsin(fin*self.wavelength[0]/4.0/np.pi)/self.pix_size)+self.xcenter[0]-self.xoff[0]
                self.pilatus1m.plotHint(self.selCutCcdData[-1],self.selCutCcdErrorData[-1],absfac=self.absfac,absnum=self.ccdSelected_AbsNum[0],cen=[self.xcenter[0]-self.xoff[0],self.ycenter[0]-self.yoff[0]],  hroi=[max(int(ini)-1,0), min(int(fin)-1,self.pilatus1m.NCOLS)],  vroi=None, ax_type='Q', wavelength=self.wavelength[0],s2d_dist=self.distance[0],sh=self.ccdSelected_Sh[0],alpha=self.alpha[0], mon=None)
                self.cutData[-1]=self.pilatus1m.hintData
            ...
            for i in self.selectedCcdFramesNums:
                if self.ui.gixCutDirComboBox.currentText()=='H Cut':
                    self.pilatus1m.plotVint(self.selCutCcdData[i],self.selCutCcdErrorData[i],absfac=self.absfac,absnum=self.ccdSelected_AbsNum[j], cen=[self.xcenter[j]-self.xoff[j],self.ycenter[j]-self.yoff[j]], hroi=None, vroi=[int(ini)-1, int(fin)-1], ax_type=str(self.ui.gixAxesComboBox.currentText()), wavelength=self.wavelength[j],s2d_dist=self.distance[j],sh=self.ccdSelected_Sh[j],alpha=self.alpha[j], mon=None)
                    self.cutData[i]=self.pilatus1m.vintData
                elif self.ui.gixCutDirComboBox.currentText()=='Qz Cut':
                    ...
                    self.pilatus1m.plotVint(self.selCutCcdData[i],self.selCutCcdErrorData[i],absfac=self.absfac,absnum=self.ccdSelected_AbsNum[j],cen=[self.xcenter[j]-self.xoff[j],self.ycenter[j]-self.yoff[j]],  hroi=None,  vroi=[fin1-1, ini1-1], ax_type='Q', wavelength=self.wavelength[j],s2d_dist=self.distance[j],sh=self.ccdSelected_Sh[j],alpha=self.alpha[j], mon=None)
                    self.cutData[i]=self.pilatus1m.vintData    
                elif self.ui.gixCutDirComboBox.currentText()=='V Cut':
                    self.pilatus1m.plotHint(self.selCutCcdData[i],self.selCutCcdErrorData[i],absfac=self.absfac,absnum=self.ccdSelected_AbsNum[j],cen=[self.xcenter[j]-self.xoff[j],self.ycenter[j]-self.yoff[j]], hroi=[int(ini)-1, int(fin)-1],  vroi=None, ax_type=str(self.ui.gixAxesComboBox.currentText()), wavelength=self.wavelength[j],s2d_dist=self.distance[j],sh=self.ccdSelected_Sh[j],alpha=self.alpha[j], mon=None)
                    self.cutData[i]=self.pilatus1m.hintData
                else:
                    ...
                    self.pilatus1m.plotHint(self.selCutCcdData[i],self.selCutCcdErrorData[i],absfac=self.absfac,absnum=self.ccdSelected_AbsNum[j], cen=[self.xcenter[j]-self.xoff[j],self.ycenter[j]-self.yoff[j]], hroi=[ini1-1, fin1-1],  vroi=None, ax_type='Q', wavelength=self.wavelength[j],s2d_dist=self.distance[j],sh=self.ccdSelected_Sh[j],alpha=self.alpha[j], mon=None)
                    self.cutData[i]=self.pilatus1m.hintData
            ...
            for i in self.selectedPilFramesNums:
                if self.ui.pilCutDirComboBox.currentText()=='H Cut':
                   ...
                   # ini1=int(2.0*self.distance[j]*np.arcsin(ini*self.wavelength[j]/4.0/np.pi)/0.172)+self.xcenter[j]
                    ...
                    self.pilatus.plotHint(self.selCutPilData[i],self.selCutPilErrorData[i],absfac=self.absfac,absnum=self.pilSelected_AbsNum[j], cen=[self.xcenter[j],self.ycenter[j]], hroi=[ini1-1, fin1-1],  vroi=None, ax_type='Q', wavelength=self.wavelength[j],s2d_dist=self.distance[j],sh=self.pilSelected_Sh[j],alpha=self.alpha[j], truealpha=self.truealpha[j], mon=None)
                    self.cutData[i]=self.pilatus.hintData
                self.cutLabel[i]=str(self.ui.imageListWidget.item(i).text().split('\t')[0])+' '+str(self.ui.imageListWidget.item(i).text().split('\t')[1])
                j=j+1
        if self.ui.gixSumCheckBox.checkState()!=0:
    ...
    def displayScanInfo(self):
        ...
        for i in self.selectedScanNums:
            ...
            try:
                ...
                impsnumtit=impsnumtit+'['+str(format(self.specPar[i]['S1L']+self.specPar[i]['S1R'],'.1f'))+', '+str(format(self.specPar[i]['S1T']+self.specPar[i]['S1B'],'.3f'))+']\t'+'['+str(format(self.specPar[i]['S4L']+self.specPar[i]['S4R'],'.1f'))+', '+str(format(self.specPar[i]['S4T']+self.specPar[i]['S4B'],'.1f'))+']\t'+'['+str(format(self.specPar[i]['S5L']+self.specPar[i]['S5R'],'.1f'))+']\n\n'
    ...
    def calg_l2(self):
       ...
       # print [self.specPar[i]['Wavelength'] for i in self.selectedScanNums]
       # print [round(self.specData[i]['L'][0],3) for i in  self.selectedScanNums]
        #self.g_l2alpha=[np.arcsin(np.round(self.specData[i]['L'][0],3)*self.specPar[i]['Wavelength']/4.0/np.pi)*180/np.pi for i in self.selectedScanNums]
    ...
    def g_l2plot(self):
        ...
        j=0
        for i in self.selectedScanNums:
            self.uig_l2.g_l2TextBrowser.append(str(i)+' \t %.4f '%self.g_l2alpha[j]+'\t %.4f'%(-np.sin(self.g_l2alpha[j]*np.pi/180)*self.g_l2[j])+'\t %.4f '%self.g_l2center[j]+'\t %.4f '%(-np.sin(self.g_l2alpha[j]*np.pi/180)*self.g_l2[j]-self.g_l2center[j]))
            j=j+1
        self.uig_l2.g_l2TextBrowser.append('\n')
        ...
        j=0
        for i in self.selectedScanNums:
            self.uig_l2.g_l2TextBrowser.append(str(i)+' \t %.4f '%self.g_l2alpha[j]+'\t %.4f'%(fp[0]-np.sin(self.g_l2alpha[j]*np.pi/180)*fp[1])+'\t %.4f '%self.g_l2center[j]+'\t %.4f '%(-np.sin(self.g_l2alpha[j]*np.pi/180)*fp[1]+fp[0]-self.g_l2center[j]))
            j=j+1
        self.uig_l2.g_l2TextBrowser.append('\n')
    ...
    def calg_l3(self):
        ...
        #self.g_l3alpha = [np.arcsin(np.round(self.specData[i]['L'][0], 3) * self.specPar[i]['Wavelength'] / 4.0 / np.pi) * 180 / np.pi for i in self.selectedScanNums]
    ...
    def g_l3plot(self):
        ...
        j=0
        for i in self.selectedScanNums:
            self.uig_l3.g_l3TextBrowser.append(str(i)+' \t %.4f '%self.g_l3alpha[j]+'\t %.4f'%(np.sin(self.g_l3alpha[j]*np.pi/180)*(self.g_l3[j]-self.g_l2[j]))+'\t %.4f '%self.g_l3center[j]+'\t %.4f '%(np.sin(self.g_l3alpha[j]*np.pi/180)*(self.g_l3[j]-self.g_l2[j])-self.g_l3center[j]))
            j=j+1
        self.uig_l3.g_l3TextBrowser.append('\n')
        ...
        j=0
        for i in self.selectedScanNums:
            self.uig_l3.g_l3TextBrowser.append(str(i)+' \t %.4f '%self.g_l3alpha[j]+'\t %.4f'%(fp[0]-np.sin(self.g_l3alpha[j]*np.pi/180)*fp[1])+'\t %.4f '%self.g_l3center[j]+'\t %.4f '%(-np.sin(self.g_l3alpha[j]*np.pi/180)*fp[1]+fp[0]-self.g_l3center[j]))
            j=j+1
        self.uig_l3.g_l3TextBrowser.append('\n')
    ...
    def curfunction(self,par,x,y):
        return y-par[0]-2*par[1]*x  
        
    def calcur(self):
        Dialog=QDialog(self)
        self.uicur=uic.loadUi('curDialog.ui', Dialog)
        self.uicur.show()
        scannum=[item for item in self.selectedScanNums]
        self.g_l3alpha=[self.specPar[i]['In_Rot'] for i in self.selectedScanNums]
        self.cur_alhpa=self.specPar[self.selectedScanNums[0]]['In_Rot']
        self.cur_gl3=self.specPar[self.selectedScanNums[0]]['g_l3']/1000.0   # in meters 
        self.curcenter=[self.scanCenter[i] for i in self.selectedScanNums]
        self.cur_shnom=-np.tan(self.cur_alhpa*np.pi/180)*self.specPar[self.selectedScanNums[0]]['g_l2'] 
        self.cursh=[self.specPar[i]['an_Sam_H'] for i in self.selectedScanNums]
        self.curoh=[self.specPar[i]['Out_Ht'] for i in self.selectedScanNums]
        self.uicur.scanLineEdit.setText(str(scannum)[1:-1])
        self.uicur.centerLineEdit.setText(str(self.curcenter)[1:-1])
        self.connect(self.uicur.calPushButton,SIGNAL('clicked()'), self.curplot)
        Dialog.exec_()
       
    def curplot(self):
        if self.uicur.centercheckBox.checkState()!=0:
            self.curcenter=map(float, str(self.uicur.centerLineEdit.text()).split(','))

        self.curx=np.array(self.cursh)-self.cur_shnom
        self.cury=np.array(self.curcenter)-np.array(self.curoh)-2.0*self.curx
        par=[0, (self.cury[-1]-self.cury[0])/(self.curx[-1]-self.curx[0])]
        p=leastsq(self.curfunction,par,args=(self.curx, self.cury),maxfev=5000)
        fp=p[0]
        
        curvature=self.cur_gl3/np.sin(self.cur_alhpa*np.pi/180)/fp[1]

        self.uicur.plotWidget.canvas.ax.clear()
        self.uicur.plotWidget.canvas.ax.plot(self.curx,self.cury,'ro')
        self.uicur.plotWidget.canvas.ax.plot(self.curx,fp[0]+2*fp[1]*self.curx,'-b',label=('curvature %.1f m'%curvature))
        self.uicur.plotWidget.canvas.ax.set_xlabel('Del_sh [mm]')
        self.uicur.plotWidget.canvas.ax.set_ylabel('Det_oh [mm]')
        self.uicur.plotWidget.canvas.ax.legend(frameon=False,scatterpoints=0,numpoints=1)
        self.uicur.plotWidget.canvas.draw()
        ...
        if  len(self.selectedplotfiles_rows)!=0: #plot plot files
            for i in range(len(self.selectedplotfiles_rows)):
                ...
                try:  #for pc
                    ...
                    datalabel=str(self.plotfiles[self.selectedplotfiles_rows[i]].split('/')[-1])
                try:
                    self.ui.PlotMplWidget.canvas.ax.errorbar(data1[:,0]*self.plotscale[i][0]+self.plotscale[i][1],data1[:,1]*self.plotscale[i][2]+self.plotscale[i][3],data1[:,2]*self.plotscale[i][2],fmt='o-',label='#'+str(self.selectedplotfiles_rows[i]+1)+' '+datalabel)
                except:
    ...
    def plotMergeDia(self):
        Dialog=QDialog(self)
        self.uiplotmerge=uic.loadUi('mcamerge.ui', Dialog)
        self.uiplotmerge.label.setText('Please provide the resolution of x')
        self.uiplotmerge.show()
        self.connect(self.uiplotmerge.cancelPushButton, SIGNAL('clicked()'), self.plotDiaClose)
        self.connect(self.uiplotmerge.okPushButton, SIGNAL('clicked()'), self.plotMerge)
            
    def plotDiaClose(self):
        self.uiplotmerge.close()
        
    
    def plotMerge(self):
        resolution=float(self.uiplotmerge.resLineEdit.text())
        data=np.loadtxt(str(self.plotfiles[self.selectedplotfiles_rows[0]]), comments='#')
        for i in range(1,len(self.selectedplotfiles_rows)):
            data1=np.loadtxt(str(self.plotfiles[self.selectedplotfiles_rows[i]]), comments='#')
       ...
       # print data[-1]
        ...
        for i in range(1,len(data)):
            if np.abs(data[i][0]-newdata[-1][0])<=resolution and i<len(data)-1:
                newdata[-1][1]=newdata[-1][1]+data[i][1]
                if m==1:
                    newdata[-1][2]=newdata[-1][2]**2+data[i][2]**2
                else:
                    newdata[-1][2] = newdata[-1][2] + data[i][2] ** 2
                m=m+1
                print m, newdata[-1][0], newdata[-1][1], newdata[-1][2]
            
            elif np.abs(data[i][0]-newdata[-1][0])<=resolution and i==len(data)-1:
                newdata[-1][1]=newdata[-1][1]+data[i][1]
                if m == 1:
                    newdata[-1][2] = newdata[-1][2] ** 2 + data[i][2] ** 2
                else:
                ...
                m=m+1
                print m
                newdata[-1][1]=newdata[-1][1]/m
                newdata[-1][2]=np.sqrt(newdata[-1][2])/m
        ...
        self.uiplotmerge.close()
        saveFileName=str(QFileDialog.getSaveFileName(caption='Save Merged Data', directory=self.directory))
        np.savetxt(saveFileName,newdata,fmt='%.4f\t%.4e\t%.4e')
        self.plotfiles.append(saveFileName)
        self.updatePlotFile()
      #  except:
          #  self.messageBox('no data to process.')    
    ...
    def peakFit(self): #fit the data with lor and gua functions and the polynomial background
        if self.mcafitstatus==0 and len(self.selectedplotfiles_rows)!=1 and self.mcafitallstatus==0:
            ...
            self.uipeakfit.show()
            #self.uipeakfit.numberOfPeakSpinBox.setvalue(3)
            try:
...
#            self.ui.PlotMplWidget.canvas.draw()
    ...
    def lorfun(self,x,p):
        return abs(p[1])/(1+(x-p[0])**2/2/p[2]**2)
...
########################  for 2d plot######################
    ...
    def update2dMaxSlider(self):
        self.ui.twodMaxLineEdit.setText(str(self.ui.twoMaxHorizontalSlider.value()*(self.imageMax-self.imageMin)/100.0))
